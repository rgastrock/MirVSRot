...
plotOnlineDependentMeasures <- function(target='inline', groups = c('far', 'mid', 'near')) {
  ...
  if (target == 'svg') {
    svglite(file='doc/fig_manuscripts/Fig4_OnlineDV.svg', width=10, height=12, pointsize=18, system_fonts=list(sans='Arial'))
  }
...
plotBlockedDependentMeasures <- function(groups = c('far', 'mid', 'near'), trialtypes = c('mir', 'washout'), quadrants = c('1', '4', '2', '1A', '1L', '1W'), target='inline'){
  ...
  if (target=='svg') {
    svglite(file='doc/fig_manuscripts/Fig5_OnlinePercentages.svg', width=7, height=11, pointsize=14, system_fonts=list(sans='Arial'))
  }
  ...
  rect(127, lim[3]-1, 146, lim[4]+1, border = "#e3e3e3", col = "#e3e3e3")
  rect(148, lim[3]-1, 168, lim[4]+1, border = "#ededed", col = "#ededed") #xleft, ybottom, x right, ytop; light grey hex code
